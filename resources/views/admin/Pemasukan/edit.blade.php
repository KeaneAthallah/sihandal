<x-app-layout>
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <!-- Header -->
            <div class="mb-6">
                <h1 class="text-3xl font-bold text-gray-800">✏️ Edit Permintaan Pemasukan</h1>
                <p class="text-gray-600 mt-1">Edit permintaan pemasukan yang masih berstatus pending</p>
            </div>

    <!-- Form Card -->
    <div class="bg-white rounded-xl shadow-md p-6">
        @php $pr = auth()->user()->role === 'admin' ? 'admin' : 'user'; @endphp
        <form method="POST" action="{{ route($pr . '.pemasukan.update', $pemasukan) }}" class="space-y-6">
            @csrf
            @method('PUT')
